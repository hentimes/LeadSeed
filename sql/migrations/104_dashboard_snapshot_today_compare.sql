-- dashboard_snapshot_today_compare
--
-- Tipo:           query permanente (reemplazo de funcion)
-- Objeto:         public.get_my_dashboard_snapshot
-- Clase:          correccion de una metrica + agregados nuevos
-- Persistencia:   permanente
-- Reversibilidad: total (volver a aplicar 102)
--
-- PROPOSITO
--
-- Dejar que las tres tarjetas de "actividad de hoy" del panel midan lo que
-- dicen medir.
--
--   completedToday    corregida: contaba tareas *creadas* hoy que estuvieran
--                     completadas, no tareas completadas hoy
--   completedCompare  nueva: sin ella la tendencia se calculaba contra un cero
--                     fijo y siempre salia "0% vs ayer" con flecha verde
--   createdToday      nueva
--   createdCompare    nueva
--
-- LA TARJETA DE RESPUESTAS
--
-- `createdToday` no es un rodeo para estimar respuestas: son leads entrados, y
-- se etiquetan asi. Las respuestas de un lead **no se registran en ninguna
-- tabla** de este proyecto; se reviso el esquema completo. La tarjeta mostraba
-- `0` fijo, que no era un numero inventado pero tampoco una medicion, y ocupaba
-- el sitio de algo que si se puede medir.
--
-- Capturar respuestas de verdad exigiria o bien un canal entrante (webhook de
-- WhatsApp, buzon IMAP) o bien pedirle al usuario que las marque a mano. Las dos
-- cosas son un frente propio, no un ajuste del panel.
--
-- DEPENDENCIAS
--
-- `public.tasks.completed_at` (migracion 103). Conserva firma, tipo de retorno y
-- todo lo que la funcion ya devolvia.
--
-- METODO
--
-- Derivada del texto de 102 con dos inserciones verificadas.
...
-- REVERSION
--
--   Volver a aplicar sql/migrations/102_dashboard_snapshot_derived_metrics.sql.
--   Ojo: eso devuelve `completedToday` a la version que filtraba por created_at.
