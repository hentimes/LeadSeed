...
-- Clase: funcion
-- Impacto: el dia del panel deja de medir 23 o 25 horas en los cambios de hora
-- Persistencia: permanente
--
-- POR QUE
--
-- La 140 arreglo el ARRANQUE del dia -se recorta en la zona de quien mira- y
-- dejo el FINAL mal: `v_today_start + interval '1 day'` sobre un
-- `timestamptz` avanza veinticuatro horas absolutas, no hasta la medianoche
-- local siguiente.
--
-- Da igual el 363 dias del ano. Los otros dos, cuando cambia la hora, el dia
-- local dura 23 o 25 horas: el rango se queda corto y pierde una hora de
-- envios, o se pasa y cuenta una hora del dia siguiente. Chile cambia la hora
-- el primer domingo de septiembre y el primero de abril.
--
-- La correccion es hacer la aritmetica en el dominio LOCAL -un `timestamp`
-- sin zona, donde "mas un dia" es la medianoche siguiente- y convertir a
-- `timestamptz` al final. `at time zone` resuelve entonces el desplazamiento
-- que toque en esa fecha concreta.
--
-- Alcanza tambien al periodo de comparacion, que tenia el mismo defecto: "ayer"
-- en un dia de cambio empezaba una hora antes o despues de la medianoche.
--
-- Lo detecto la revision automatica del PR #1.
--
-- METODO
--
-- Derivada del texto de la 140. La firma no cambia, asi que no hace falta
-- retirar la funcion: `create or replace` la sustituye y los permisos que la
-- 141 le puso se conservan.
...
declare
  ...
  /*
   * El dia local, como `timestamp` SIN zona: es el dominio donde "un dia mas"
   * significa la medianoche siguiente y no veinticuatro horas exactas.
   */
