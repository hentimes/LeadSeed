-- Query permanente
-- Dominio: panel / metricas del dia
-- Objetivo: que "hoy" sea el dia de quien mira y no el dia UTC, y no contar envios borrados
--
-- SINTOMA
--
-- El contador de mensajes del panel se vaciaba de noche: se enviaba durante
-- todo el dia y a partir de cierta hora marcaba cero.
--
-- CAUSA
--
-- `v_today_start := date_trunc('day', now())`. `now()` es timestamptz y
-- `date_trunc` lo corta en la zona de la SESION, que en Supabase es UTC. Chile
-- va cuatro horas por detras, asi que a las 20:00 hora local ya es el dia
-- siguiente en UTC: el rango pasaba a empezar en ese instante y todo lo enviado
-- ese mismo dia, desde la manana, quedaba fuera. No era que faltaran envios,
-- era que el dia terminaba cuatro horas antes de tiempo.
--
-- La zona la manda el cliente. Es el unico que sabe cual es el dia de quien
-- mira, y ya la tiene sin preguntar nada
-- (`Intl.DateTimeFormat().resolvedOptions().timeZone`). El default 'UTC' deja
-- intacta cualquier llamada que no la pase.
--
-- La correccion alcanza tambien a los leads creados hoy y al periodo de
-- comparacion, que se derivan del mismo `v_today_start`. Es lo correcto: eran
-- el mismo dia mal cortado.
--
-- DE PASO
--
-- Los conteos de `send_logs` no excluian `deleted_at`. La 135 agrego el
-- borrado logico y dejo dicho que esta tabla alimenta las metricas del panel,
-- pero esta consulta es anterior y nunca se actualizo: un envio borrado del
-- historial seguia sumando. Ese error va en el sentido contrario al del
-- sintoma -contaba de mas- y por eso no se notaba.
--
-- METODO
--
-- Derivada del texto vigente de la 104, con las inserciones de arriba.

-- La firma cambia, asi que `create or replace` NO sustituye: crearia una
-- sobrecarga y convivirian las dos. Con el parametro nuevo defaulteado, una
-- llamada con un solo argumento quedaria ambigua y PostgREST fallaria con
-- "function is not unique". Se retira la vieja antes de crear la nueva.
drop function if exists public.get_my_dashboard_snapshot(text);
...
as $$
declare
  -- Zona de quien mira el panel. El default 'UTC' conserva el comportamiento
  -- anterior en cualquier llamada que no la mande.
...
-- REVERSION
--
--   Volver a aplicar sql/migrations/104_dashboard_snapshot_today_compare.sql,
--   y borrar antes la firma de dos parametros:
