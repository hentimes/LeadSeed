...
# We need to find the start of the return block to replace it.
...
# For safety, let's keep the injection before return if it doesn't exist, but since it already exists from Round 3, we should find where it starts if possible.
# Actually, since Round 3 injected it right before the return block, it's safer to just replace from `useEffect(() => { document.body.style.overflow` if it exists.
use_effect_idx = content.find("useEffect(() => {\n    document.body.style.overflow = 'hidden';")
if use_effect_idx != -1:
    start_idx = use_effect_idx - 2 # replace everything from the use_effect
...
new_return_and_hooks = """  useEffect(() => {
  ...
  return (
    <div onClick={handleBackdropClick} className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-start justify-center pt-8 p-4 overflow-y-auto custom-scrollbar">
      <div className="bg-white rounded-[8px] shadow-2xl w-full max-w-[420px] flex flex-col mx-auto animate-scale-in border border-slate-100 mb-8 relative">
        
        {/* Header */}
        <div className="flex items-start justify-between p-4 pb-3 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  className="px-2 py-0.5 rounded-[4px] text-[10px] font-bold tracking-wide"
        ...
        <div className="flex-1 p-4 pt-3 space-y-4">
          ...
          <div className="flex flex-col gap-2">
            {lead.phone && (
              <div className="flex items-center gap-2">
                <div 
                  className="flex-1 flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-[6px] cursor-pointer hover:bg-slate-100 transition-colors"
                  title="Doble clic para copiar"
                  onDoubleClick={() => copyToClipboard(lead.phone)}
                >
                  <span className="text-slate-400">{Icon.Phone()}</span>
                  <span className="text-[12px] font-semibold text-slate-700">{lead.phone}</span>
                </div>
                <a href={`https://wa.me/${lead.phone.replace(/[^+\\d]/g, '')}`} target="_blank" rel="noopener noreferrer" className="w-8 h-8 flex items-center justify-center bg-[#F2EEFF] text-[#6C4CF6] rounded-[6px] hover:bg-[#E0D4FF] transition-colors border border-[#E0D4FF]" title="WhatsApp">
                  {Icon.Phone()}
                </a>
                <a href={`tel:${lead.phone.replace(/[^+\\d]/g, '')}`} className="w-8 h-8 flex items-center justify-center bg-slate-100 text-slate-700 rounded-[6px] hover:bg-slate-200 transition-colors border border-slate-200" title="Llamar">
                  {Icon.Phone()}
                </a>
              </div>
            )}
            {lead.email && (
              <div className="flex items-center gap-2">
                ...
                >
                  <span className="text-slate-400">{Icon.Email()}</span>
                  <span className="text-[12px] font-semibold text-slate-700 truncate">{lead.email}</span>
                </div>
                <a href={`mailto:${lead.email}`} className="w-8 h-8 flex items-center justify-center bg-slate-100 text-slate-700 rounded-[6px] hover:bg-slate-200 transition-colors border border-slate-200" title="Enviar correo">
                  {Icon.Email()}
                </a>
              </div>
            )}
          ...
          {(isPlanesproLead || journey) && (
            <div>
                    ...
                    <div className="mt-3 space-y-3 relative ml-2">
                      ...
                      {journey.motivo && (
                        <div className="relative pl-4">
                          ...
                          <p className="text-[12px] font-medium text-slate-800 leading-snug">{toJourneyLabel(String(journey.motivo), STEP1_MOTIVO_LABELS)}</p>
                        </div>
                      )}
                      ...
                      {journey.necesidad && (
                        <div className="relative pl-4">
                          ...
                          <p className="text-[12px] font-medium text-slate-800 leading-snug">{toJourneyLabel(String(journey.necesidad), STEP1_NECESIDAD_LABELS)}</p>
                        </div>
                      )}
                      ...
                      {journey.objetivo && (
                        <div className="relative pl-4">
                          ...
                          <p className="text-[12px] font-medium text-slate-800 leading-snug">{toJourneyLabel(String(journey.objetivo), STEP1_OBJETIVO_LABELS)}</p>
                        </div>
                      )}
                    </div>
                  </details>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {/* Renta condicional: siempre mostrar si existe */}
                {(planesproDetails.rangoRenta || rawPayload.renta || rawPayload.renta_liquida) && (
                  <div className="bg-slate-50 p-2 rounded-[6px] border border-slate-200">
                    ...
                    <p className="text-[12px] font-semibold text-slate-700">{String(planesproDetails.rangoRenta || toReadableValue(rawPayload.renta) || toReadableValue(rawPayload.renta_liquida))}</p>
                  </div>
                )}
              ...
              {/* Información adicional de Cargas si existe */}
              {(planesproDetails.numeroCargas && planesproDetails.numeroCargas !== '0') && (
                <details className="mt-2 group">
                  <summary className="text-[11px] font-bold text-slate-500 cursor-pointer hover:text-slate-700 list-none flex items-center gap-1 bg-slate-50 p-2 rounded-[6px] border border-slate-200">
                    <span className="group-open:rotate-90 transition-transform">{Icon.ChevronRight()}</span> 
                    Familia / Cargas ({planesproDetails.numeroCargas})
                  </summary>
                  <div className="mt-1 p-2 bg-slate-50 border border-slate-200 rounded-[6px]">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide mb-1">Edades de cargas</p>
                    <p className="text-[12px] font-semibold text-slate-700">
                      {planesproDetails.edadesCargas && planesproDetails.edadesCargas.length > 0 
                        ? planesproDetails.edadesCargas.join(', ') 
                        : 'No especificado'}
                    </p>
                  </div>
                </details>
          ...
          <div>
            ...
            {!canCreateAppointment ? (
              <div className="bg-slate-50 border border-slate-200 rounded-[6px] p-2.5 flex justify-between items-center">
                <div>
                  <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wide">{String(visibleAppointmentStatus)}</p>
                  <p className="text-[12px] font-semibold text-slate-800">{formatAppointmentDate(String(visibleAppointmentAt))}</p>
                </div>
                {onNavigate && (
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Notas internas</h3>
            ...
            <div className="flex gap-2 mb-2">
              ...
              />
              <button onClick={addNote} className="px-3 py-1.5 bg-[#F2EEFF] text-[#6C4CF6] text-[12px] font-bold rounded-[6px] hover:bg-[#E0D4FF] transition-colors">
                {Icon.Send()}
              </button>
            </div>
            ...
            {showNotes && (
              <div className="space-y-2 mt-2">
                {notes.map((note) => (
                  <div key={note.id} className="bg-slate-50 rounded-[6px] p-2.5 border border-slate-200">
...
print("Premium Redesign Applied Round 4")
