...
export interface ProfileSettingsRow {
  ...
  daily_send_task_enabled: boolean | null;
  active_smart_lists: string[] | null;
  list_groups: { id: string; name: string; listIds: (number | string)[] }[] | null;
}

const PROFILE_SETTINGS_SELECT =
  'compact_mode, dark_mode, visible_cols, email_provider, resend_from_name, resend_from_email, export_format, whatsapp_daily_limit, daily_goal_whatsapp, daily_goal_email, daily_goal_calls, dashboard_compare_period, whatsapp_client_preference, hide_unnamed_leads, daily_send_task_enabled, active_smart_lists, list_groups';
