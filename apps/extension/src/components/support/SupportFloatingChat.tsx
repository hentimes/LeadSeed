...
export default function SupportFloatingChat() {
  ...
  const handleBump = async () => {
    ...
    await createSupportMessage(user.id, targetAdminId, 'Reenviado: el usuario solicita atención a este ticket.');
    setIsBumping(false);
  };
        ...
        <div className="fixed bottom-6 left-4 right-[calc(var(--ls-rail-width,0px)+1rem)] h-96 bg-surface rounded-2xl shadow-2xl flex flex-col overflow-hidden z-20 border border-purple-100">
          ...
          <form onSubmit={handleSend} className="p-3 border-t border-line bg-surface">
            <div className="flex gap-2 items-center">
              <input type="text" value={newMessage} onChange={handleTyping} placeholder="Escribe aquí..." className="flex-1 bg-surface-muted border border-line rounded-full px-4 py-2 text-xs outline-none focus:border-purple-400 focus:ring-1 focus:ring-purple-400 text-ink-secondary" />
