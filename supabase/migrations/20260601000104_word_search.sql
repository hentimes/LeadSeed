-- 063 - Buscar por palabras sueltas
--
-- El buscador comparaba la frase entera contra cada campo, asi que "Henry
-- Farias" no encontraba a "Henry Jose Farias Pacheco": el segundo nombre
-- rompia la coincidencia. Con nombres compuestos, que es lo normal en Chile,
-- el buscador quedaba practicamente inutilizable.
--
-- Ahora cada palabra tiene que aparecer en algun dato del lead, sin importar
-- en cual ni en que orden. El lado de PostgREST se arregla en
-- leadsRepository.ts con el mismo criterio (un .or() por palabra).
--
-- Solo cambia el filtro de busqueda. El criterio de olvidado, el resto de los
-- filtros, el orden y el tipo de retorno quedan como en la migracion 061.
  ...
  filtered as (
    ...
    where (p_list_id is null or coalesce(f.lista_ids, '{}'::integer[]) @> array[p_list_id::integer])
      ...
      and (
        ...
        or (
          select bool_and(
            concat_ws(' ', f.name, f.email, f.phone, f.company, f.rut)
              ilike ('%' || token || '%')
          )
