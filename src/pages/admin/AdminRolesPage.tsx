...
export default function AdminRolesPage() {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Cargando perfiles...</div>;
    ...
    <div className="flex gap-6 h-full">
      ...
      <div className="w-1/3 bg-white border border-gray-200 rounded-xl shadow-sm flex flex-col overflow-hidden">
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
          <h3 className="font-bold text-gray-800">Perfiles / Planes</h3>
        ...
        <div className="overflow-y-auto flex-1 p-2 space-y-2">
          {plans.map(p => (
            <div 
              ...
              className={`p-4 rounded-lg border cursor-pointer transition-colors ${selectedPlan?.id === p.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}
            >
              <h4 className="font-bold text-gray-900">{p.name}</h4>
              <p className="text-xs text-gray-500 mt-1">{p.description || 'Sin descripción'}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        {showNewPlan ? (
          <div className="p-6">
            ...
            <form onSubmit={handleCreatePlan} className="space-y-4 max-w-md">
              <div>
                ...
                <input type="text" required value={newPlanName} onChange={e => setNewPlanName(e.target.value)} className="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Ej. Pro Anual" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
                <textarea value={newPlanDesc} onChange={e => setNewPlanDesc(e.target.value)} className="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Beneficios del plan..."></textarea>
              </div>
              <div className="flex gap-2">
                ...
                <button type="button" onClick={() => setShowNewPlan(false)} className="text-gray-600 hover:text-gray-900 px-4 py-2 text-sm font-medium">Cancelar</button>
              </div>
            </form>
          </div>
        ) : selectedPlan ? (
          <>
            ...
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <p className="text-sm text-gray-600 mb-6">Selecciona las funcionalidades que vendrán incluidas por defecto para todos los usuarios que tengan este plan asignado.</p>
                ...
                {features.map(f => {
                  ...
                  return (
                    <div 
                      ...
                      className={`border rounded-xl p-4 cursor-pointer flex gap-4 transition-all ${isAssigned ? 'border-blue-500 bg-blue-50/50 shadow-sm' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'}`}
                    >
                      <div className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded border flex items-center justify-center ${isAssigned ? 'bg-blue-600 border-blue-600' : 'bg-white border-gray-300'}`}>
