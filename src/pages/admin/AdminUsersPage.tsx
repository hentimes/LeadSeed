...
export default function AdminUsersPage() {
  ...
  return (
    <div className="flex gap-6 h-full">
      {/* Lista de Usuarios */}
      <div className="w-1/3 bg-surface border border-line rounded-xl shadow-sm flex flex-col overflow-hidden">
        <div className="p-3 border-b border-line bg-surface-muted flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2">
              {isAdmin && (
                <input 
                  ...
                  className="w-4 h-4 text-blue-600 rounded border-line-strong cursor-pointer"
                />
              )}
              <h3 className="font-bold text-ink text-sm">Usuarios ({profiles.length})</h3>
            </div>
            ...
            {isAdmin && selectedUserIds.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-48 bg-surface rounded-xl shadow-lg border border-line opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                    <button onClick={() => handleBulkAction('helper')} className="w-full text-left px-4 py-2 text-xs hover:bg-surface-muted flex items-center gap-2"><Icon.CheckCircle /> Hacer Helper</button>
                    <button onClick={() => handleBulkAction('remove_helper')} className="w-full text-left px-4 py-2 text-xs hover:bg-surface-muted flex items-center gap-2 text-red-600"><Icon.Close /> Quitar Helper</button>
                    <div className="h-px bg-surface-hover my-1"></div>
                    <button onClick={() => handleBulkAction('ban')} className="w-full text-left px-4 py-2 text-xs hover:bg-surface-muted flex items-center gap-2 text-ink-muted"><Icon.Warning /> Banear Usuarios</button>
                    <button onClick={() => handleBulkAction('promo')} className="w-full text-left px-4 py-2 text-xs hover:bg-surface-muted flex items-center gap-2 text-ink-muted"><Icon.Plus /> Añadir Promoción</button>
                    <button onClick={() => handleBulkAction('banner')} className="w-full text-left px-4 py-2 text-xs hover:bg-surface-muted flex items-center gap-2 text-ink-muted"><Icon.View /> Activar Banners</button>
                    <div className="h-px bg-surface-hover my-1"></div>
                    <button onClick={() => handleBulkAction('group')} className="w-full text-left px-4 py-2 text-xs hover:bg-surface-muted flex items-center gap-2 text-ink-muted"><Icon.Users /> Crear Grupo/Lista</button>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="overflow-y-auto flex-1">
          {profiles.length === 0 ? (
            <p className="p-4 text-sm text-ink-muted text-center">No hay usuarios registrados.</p>
          ) : (
            profiles.map(p => {
              ...
              return (
                <div 
                  ...
                  className={`p-2.5 border-b border-line cursor-pointer hover:bg-surface-muted transition-colors flex items-center gap-3 ${selectedUser?.id === p.id ? 'bg-blue-50/50' : ''}`}
                >
                  {isAdmin && (
                    <input 
                      type="checkbox"
                  ...
                  <div className="flex-1 min-w-0 flex justify-between items-center">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="relative shrink-0">
                        {p.avatar_url ? (
                          <img src={p.avatar_url} alt="Avatar" className="w-8 h-8 rounded-full border border-line" />
                        ) : (
                      ...
                      <div className="flex flex-col justify-center min-w-0 pr-2">
                        <div className="flex items-center">
                          <p className="text-[13px] font-bold text-ink truncate leading-tight">{p.full_name || p.email.split('@')[0]}</p>
                        ...
                        <p className="text-[11px] text-ink-muted truncate leading-tight mt-0.5" title={p.email}>{p.email}</p>
                      </div>
                    </div>
                    
                    <div className="flex flex-col items-end gap-1 shrink-0 ml-2">
                      <span className={`px-1.5 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider ${plan ? 'bg-indigo-50 text-indigo-700' : 'bg-surface-hover text-ink-muted'}`}>
                        {plan ? plan.name : 'Sin Plan'}
                      </span>
                      <span className="text-[10px] text-ink-muted font-medium capitalize">
      ...
      {selectedUser ? (
        <div className="flex-1 bg-surface border border-line rounded-xl shadow-sm flex flex-col overflow-hidden">
          <div className="p-6 border-b border-line bg-surface-muted flex justify-between items-start">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-ink mb-1">{selectedUser.full_name || selectedUser.email}</h2>
                <p className="text-sm text-ink-muted font-mono">{selectedUser.email} | {selectedUser.id}</p>
                {onlineUsers[selectedUser.id] ? (
                  ...
                  <p className="text-xs text-ink-muted mt-1">Última conexión: {new Date(selectedUser.last_seen_at).toLocaleString('es-CL')}</p>
                ) : (
                  <p className="text-xs text-ink-muted mt-1">Nunca se ha conectado</p>
                )}
              </div>
            </div>
          </div>
