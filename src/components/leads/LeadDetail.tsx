      ...
      <div className="bg-white dark:bg-slate-800/80 dark:backdrop-blur-md dark:bg-gray-900 rounded-lg shadow-xl w-[92%] max-w-[340px] max-h-[90vh] flex flex-col mx-auto animate-scale-in">
        ...
        <div className="flex items-center justify-between p-4 border-b shrink-0">
          ...
          <div className="flex gap-2">
            ...
            }} id="copy-btn" className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300 text-sm flex items-center gap-1"><Icon.Copy /> Copiar</button>
            <button onClick={() => { onEdit(lead); onClose(); }} className="text-blue-600 hover:text-blue-800 text-sm">{Icon.Edit()} Editar</button>
            <button onClick={onClose} className="text-gray-400 hover:text-slate-500 dark:text-slate-400 text-lg leading-none ml-2">&times;</button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            {lead.phone && <div><span className="text-slate-400 dark:text-slate-500 text-xs">Teléfono</span><p>{lead.phone}</p></div>}
            ...
            <div><span className="text-slate-400 dark:text-slate-500 text-xs">Actualizado</span><p>{new Date(lead.updatedAt).toLocaleDateString('es-CL')}</p></div>
          </div>

          {/* Renderizar Metadata (Campos Dinámicos) */}
          {lead.metadata && Object.keys(lead.metadata).length > 0 && (
            <div className="mt-2">
              <p className="text-xs text-slate-400 dark:text-slate-500 mb-1">Información Adicional</p>
