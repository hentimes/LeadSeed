...
export default function GoalsSettings() {
  ...
  return (
    <div className="bg-transparent pt-2 animate-fade-in">
      <div className="mb-4">
        <h3 className="text-lg font-bold text-gray-800">Metas y Dashboard</h3>
        <p className="text-xs text-gray-500 mt-1">Configura tus objetivos diarios y la comparativa del panel.</p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Metas */}
          <div>
            <h4 className="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-3">Cuotas Diarias</h4>
            <div className="flex flex-col gap-3">
              <label className="flex items-center justify-between text-xs text-gray-600">
                WhatsApp / día
                <input type="number" min="0" value={waGoal} onChange={e => setWaGoal(Number(e.target.value))} className="w-20 border-b border-gray-300 px-2 py-1 text-right focus:border-blue-500 outline-none bg-transparent" />
              </label>
              <label className="flex items-center justify-between text-xs text-gray-600">
                Emails / día
                <input type="number" min="0" value={emailGoal} onChange={e => setEmailGoal(Number(e.target.value))} className="w-20 border-b border-gray-300 px-2 py-1 text-right focus:border-blue-500 outline-none bg-transparent" />
              </label>
              <label className="flex items-center justify-between text-xs text-gray-600">
                Llamadas / día
                <input type="number" min="0" value={callGoal} onChange={e => setCallGoal(Number(e.target.value))} className="w-20 border-b border-gray-300 px-2 py-1 text-right focus:border-blue-500 outline-none bg-transparent" />
              </label>
            </div>
          </div>
          ...
          {/* Comparación */}
          <div>
            <h4 className="text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-3">Análisis de Progreso</h4>
            <div className="flex flex-col gap-3">
              <label className="flex flex-col text-xs text-gray-600">
                ...
                <select 
                  ...
                  className="border-b border-gray-300 px-2 py-1.5 focus:border-blue-500 outline-none bg-transparent w-full font-medium"
                >
