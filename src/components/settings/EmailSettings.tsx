    ...
    <div className="animate-fade-in pt-2">
      ...
      <div className="mb-6">
        ...
        <select 
          ...
          className="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm outline-none"
        >
      ...
      {emailSettings.provider === 'resend' ? (
        <div className="space-y-4 p-4 bg-gray-50 border rounded-xl">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">API Key <span className="text-gray-400 font-normal text-xs">(Empieza con re_...)</span></label>
            <input type="password" value={emailSettings.resendApiKey} onChange={(e) => setEmailSettings({ ...emailSettings, resendApiKey: e.target.value })}
              className="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm outline-none" placeholder="re_123456789..." />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={emailSettings.resendFromName} onChange={(e) => setEmailSettings({ ...emailSettings, resendFromName: e.target.value })}
                className="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm outline-none" placeholder="Ej: Juan Pérez" />
            </div>
            <div>
              ...
              <input type="email" value={emailSettings.resendFromEmail} onChange={(e) => setEmailSettings({ ...emailSettings, resendFromEmail: e.target.value })}
                className="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm outline-none" placeholder="correo@tudominio.com" />
            </div>
          </div>
          <p className="text-xs text-gray-500 italic mt-1">Asegúrate de que el dominio del correo remitente esté verificado en tu cuenta de Resend.</p>
        </div>
      ) : (
        <div className="space-y-4 p-4 bg-gray-50 border rounded-xl">
          <p className="text-sm text-gray-600">
            Configuración de <a href="https://www.emailjs.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-medium">EmailJS</a>. Campos requeridos en la plantilla: <code className="bg-white border px-1.5 py-0.5 rounded text-gray-700 text-xs mx-1">to_email, to_name, subject, message, message_html</code>.
          </p>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Public Key (User ID)</label>
            <input type="text" value={emailSettings.userId} onChange={(e) => setEmailSettings({ ...emailSettings, userId: e.target.value })}
              className="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm outline-none" /></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Service ID</label>
              <input type="text" value={emailSettings.serviceId} onChange={(e) => setEmailSettings({ ...emailSettings, serviceId: e.target.value })}
                className="w-full border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-blue-500 focus:border-blue-500 shadow-sm outline-none" /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Template ID</label>
