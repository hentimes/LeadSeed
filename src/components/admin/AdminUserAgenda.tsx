...
export default function AdminUserAgenda({ selectedUser }: Props) {
  ...
  if (loading) {
    return <div className="p-8 text-center text-ink-muted animate-pulse">Cargando agenda observada...</div>;
  }
  ...
  return (
    <div className="flex flex-col h-full bg-surface">
      <div className="px-4 py-3 border-b border-line bg-surface-muted">
        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-ink-secondary">Agenda observada</p>
      ...
      <div className="grid grid-cols-3 gap-3 px-4 py-3 border-b border-line bg-surface">
        <div className="rounded-lg border border-line px-3 py-2">
          <p className="text-[11px] uppercase tracking-[0.14em] text-ink-muted">Total</p>
          <p className="mt-1 text-lg font-bold text-ink">{appointments.length}</p>
        </div>
        <div className="rounded-lg border border-line px-3 py-2">
          <p className="text-[11px] uppercase tracking-[0.14em] text-ink-muted">Activas</p>
          <p className="mt-1 text-lg font-bold text-ink">{activeAppointments.length}</p>
        </div>
        <div className="rounded-lg border border-line px-3 py-2">
          <p className="text-[11px] uppercase tracking-[0.14em] text-ink-muted">Cerradas</p>
          <p className="mt-1 text-lg font-bold text-ink">{closedAppointments.length}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto px-4 py-4 space-y-5">
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-ink">Activas</h3>
            <span className="text-[11px] text-ink-muted">{activeAppointments.length}</span>
          </div>
          {activeAppointments.length === 0 ? (
            <p className="text-sm text-ink-muted">Sin citas activas en el rango observado.</p>
          ) : (
            activeAppointments.map((appointment) => {
              ...
              return (
                <article
                  key={appointment.id}
                  className="border border-line rounded-lg px-3 py-3 bg-surface"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-ink truncate">{appointment.leadName}</p>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-ink-secondary">
                    <div>
                      <span className="block uppercase tracking-[0.12em] text-ink-muted">Canal</span>
                      <span>{appointment.sourceChannel}</span>
                    </div>
                    <div>
                      <span className="block uppercase tracking-[0.12em] text-ink-muted">Capture ref</span>
                      <span>{appointment.captureRef || '-'}</span>
                    </div>
                  </div>

                  {appointment.notes && (
                    <p className="mt-3 text-xs text-ink-secondary whitespace-pre-wrap">{appointment.notes}</p>
                  )}
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-ink">Cerradas</h3>
            <span className="text-[11px] text-ink-muted">{closedAppointments.length}</span>
          </div>
          {closedAppointments.length === 0 ? (
            <p className="text-sm text-ink-muted">Sin citas cerradas en el rango observado.</p>
          ) : (
            closedAppointments.map((appointment) => (
              <article
                key={appointment.id}
                className="border border-line rounded-lg px-3 py-3 bg-surface-muted/80"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-ink truncate">{appointment.leadName}</p>
                  ...
                  <span className="shrink-0 rounded-md bg-surface-sunken text-ink px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em]">
                    {appointment.status}
                  </span>
                </div>
              </article>
