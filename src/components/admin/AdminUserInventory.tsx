...
export default function AdminUserInventory({ selectedUser }: Props) {
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-500 animate-pulse">Cargando inventario de la nube...</div>;
  }
      ...
      <div>
        ...
        {leads.length === 0 ? (
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
            <p className="text-sm text-gray-500">Este usuario no tiene leads asignados en la nube.</p>
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {leads.map(lead => (
                  <tr key={lead.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{lead.company || '-'}</td>
                    <td className="px-4 py-3 whitespace-nowrap">
