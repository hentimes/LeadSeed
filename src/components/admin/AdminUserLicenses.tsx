  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <section>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-700/50">
          <select 
            value={selectedUser.plan_id || ''} 
            onChange={(e) => onUpdatePlan(e.target.value)}
            className="w-full sm:w-1/2 border border-slate-300 dark:border-slate-600/50 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
          >
          ...
          <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">El usuario heredará todas las funcionalidades incluidas en este plan.</p>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Añade accesos extra a funcionalidades que no están en su plan, o asigna días de prueba.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {features.map(f => {
            ...
            return (
              <div key={f.id} className={`border rounded-lg p-4 flex flex-col justify-between ${isAssigned ? (isTrial ? 'border-amber-300 bg-amber-50' : 'border-green-300 bg-green-50') : 'border-slate-200 dark:border-slate-700/50 hover:border-blue-300 transition-colors'}`}>
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500 line-clamp-2">{f.description || 'Sin descripción'}</p>
