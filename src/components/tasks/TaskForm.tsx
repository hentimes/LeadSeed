...
export default function TaskForm({ task, leads, lists, onSave, onCancel }: TaskFormProps) {
  ...
  return (
    <div className="mb-4 p-4 border rounded-lg bg-surface-muted dark:border-gray-700">
      <h3 className="text-base font-semibold mb-3">{task ? 'Editar Tarea' : 'Nueva Tarea'}</h3>
      <div className="space-y-3">
        <div>
          <label className="block text-sm font-medium text-ink-secondary mb-1">Título *</label>
        ...
        <div>
          <label className="block text-sm font-medium text-ink-secondary mb-1">Vencimiento</label>
          <input type="date" value={fechaVencimiento} onChange={(e) => setFechaVencimiento(e.target.value)}
            className="w-full border rounded px-3 py-2 text-sm dark:bg-gray-900 dark:border-gray-600" />
        </div>
        <div>
          <label className="block text-sm font-medium text-ink-secondary mb-1">Leads asignados</label>
          <div className="border rounded max-h-32 overflow-y-auto p-1 dark:border-gray-600">
            {leads.filter((l) => !l.deletedAt).map((lead) => (
              <label key={lead.id} className="flex items-center gap-1.5 px-1 py-0.5 hover:bg-surface-hover rounded cursor-pointer text-xs">
        ...
        {lists.length > 0 && (
          <div>
            <label className="block text-sm font-medium text-ink-secondary mb-1">Listas asignadas</label>
            <div className="flex flex-wrap gap-1">
              {lists.map((list) => (
                <button key={list.id} type="button" onClick={() => setLeadListIds((prev) => prev.includes(list.id!) ? prev.filter((x) => x !== list.id) : [...prev, list.id!])}
                  className={`px-2 py-1 rounded-full text-xs font-medium border ${leadListIds.includes(list.id!) ? 'text-white border-transparent' : 'text-ink-secondary border-line-strong'}`}
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button onClick={onCancel} className="text-ink-muted hover:text-ink-secondary px-3 py-1.5 rounded text-xs font-medium transition-colors underline decoration-dotted">
            Cancelar
          </button>
        </div>
      </div>
