...
export default function TaskCard({
  ...
  return (
    <div className={`border rounded-lg p-3 ${task.status === 'completada' ? 'opacity-60 bg-surface-muted' : isOverdue ? 'border-red-300 bg-red-50 dark:border-red-800' : isToday ? 'border-amber-300 bg-amber-50 dark:border-amber-800' : 'bg-surface dark:backdrop-blur-md dark:border-gray-700'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h4 className={`text-sm font-medium ${task.status === 'completada' ? 'line-through text-ink-muted' : ''}`}>{task.titulo}</h4>
          ...
          {task.descripcion && <p className="text-xs text-ink-secondary mt-1">{task.descripcion}</p>}
          {task.fechaVencimiento && (
            <p className={`text-xs mt-1 ${isOverdue ? 'text-red-600 font-medium' : isToday ? 'text-amber-600 font-medium' : 'text-ink-muted'}`}>
          ...
          <div className="flex gap-1 mt-1 flex-wrap">
            ...
            {task.leadListIds?.map((lid) => (
              <span key={lid} className="text-[10px] bg-surface-hover text-ink-secondary px-1 rounded">{lists.find((l) => l.id === lid)?.name || '?'}</span>
            ))}
          </div>
        </div>
        <div className="flex gap-1 shrink-0">
          <button onClick={() => onEdit(task)} className="text-ink-muted hover:text-blue-600 text-xs" title="Editar" aria-label={`Editar ${task.titulo}`}>{Icon.Edit()}</button>
          <button onClick={() => onDelete(task.id!)} className="text-ink-muted hover:text-red-600 text-xs" title="Eliminar" aria-label={`Eliminar ${task.titulo}`}>{Icon.Trash()}</button>
        </div>
      </div>
    </div>
  );
