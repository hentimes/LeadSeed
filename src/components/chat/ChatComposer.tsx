...
export default function ChatComposer({
  ...
  return (
  <div className="p-4 bg-surface border-t border-line dark:border-gray-800 z-10 flex flex-col">
    {cleanupResult && (
    ...
    {replyTo && (
      <div className="mb-3 pl-3 border-l-2 border-primary flex justify-between items-center bg-surface-muted dark:bg-gray-800 py-1.5 pr-2 rounded-r-md text-sm shadow-sm transition-all">
        <div className="flex flex-col overflow-hidden">
          ...
          <span className="text-ink truncate text-xs mt-0.5">
            {toPlainText(replyTo.content)}
          </span>
        </div>
        <button onClick={() => setReplyTo(null)} className="p-1 text-ink-muted hover:text-ink transition-colors">
          <Icon.Close />
        </button>
      </div>
    )}
    ...
    <form onSubmit={handleSend} className="relative">
      ...
      {attachment.file && (
        <div className="flex items-center gap-2 mb-2 px-2 py-1.5 rounded-xl bg-surface-muted dark:bg-gray-800 border border-line dark:border-gray-700">
          {attachment.previewUrl ? (
            ...
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-surface text-ink-muted flex-shrink-0">
          ...
          <button
            ...
            className="p-1 rounded-full text-ink-muted hover:bg-surface hover:text-ink transition-colors flex-shrink-0"
      ...
      <div className="flex items-end gap-1 border border-line dark:border-gray-700 bg-surface rounded-[20px] px-2 py-1.5 focus-within:ring-2 focus-within:ring-primary-soft transition-all shadow-sm">
        ...
        <button
          ...
          className="p-2 mb-0.5 text-ink-muted hover:bg-surface-muted rounded-full transition-colors flex-shrink-0"
        ...
        {isAnnouncementDraft && (
          <span
            className="p-2 mb-0.5 rounded-full text-amber-600 flex-shrink-0 animate-fade-in"
        ...
        <button
          ...
          className="w-10 h-10 mb-0.5 ml-1 bg-primary hover:bg-primary-hover disabled:bg-line disabled:text-ink-muted text-white rounded-[14px] flex items-center justify-center flex-shrink-0 transition-all transform active:scale-95"
        >
