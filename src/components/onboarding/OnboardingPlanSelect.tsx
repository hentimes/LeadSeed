...
export default function OnboardingPlanSelect() {
  ...
  if (loading) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-surface-muted flex-col gap-4">
        <div className="w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
        <p className="text-ink-muted font-medium">Cargando planes...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-surface-muted flex flex-col items-center py-10 px-4">
      ...
      <div className="w-full max-w-sm mb-6 text-center">
        ...
        <h1 className="text-2xl font-bold text-ink">Bienvenido a PlanesPro</h1>
        <p className="text-sm text-ink-muted mt-2">Para comenzar, elige el plan que mejor se adapte a tus necesidades de ventas.</p>
      </div>
      ...
      <div className="w-full max-w-sm space-y-4">
          ...
          return (
            <div 
              ...
              className={`relative overflow-hidden rounded-2xl border-2 transition-all p-5 
                ${isEnabled 
                  ? 'border-blue-500 bg-surface dark:backdrop-blur-md shadow-md cursor-pointer hover:border-blue-600 hover:shadow-lg' 
                  : 'border-line bg-surface-muted cursor-not-allowed opacity-80'}`}
            >
              {!isEnabled && (
                <div className="absolute top-4 right-4">
                  <span className="bg-surface-sunken text-ink-secondary text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide">
                    Próximamente
                  </span>
                </div>
              )}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className={`text-lg font-bold ${isEnabled ? 'text-ink' : 'text-ink-muted'}`}>
                    {plan.name}
                  </h3>
                  <p className={`text-sm mt-1 leading-relaxed ${isEnabled ? 'text-ink-secondary' : 'text-ink-muted'}`}>
        ...
        {plans.length === 0 && (
          <div className="text-center p-6 border-2 border-dashed border-line-strong rounded-xl">
            <p className="text-ink-muted text-sm">No hay planes configurados en el sistema.</p>
          </div>
        )}
      </div>

      <div className="mt-8">
        <button onClick={signOut} className="text-sm text-ink-muted hover:text-ink-secondary font-medium underline">
          Cerrar Sesión
        </button>
      </div>
    </div>
