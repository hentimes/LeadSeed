...
export default function LeadDetail({ lead, lists, onClose, onEdit }: Props) {
  ...
  useEffect(() => {
    (async () => {
      const [n, s, wt, et] = await Promise.all([
        db.leadNotes.where('leadId').equals(lead.id!).reverse().sortBy('createdAt'),
        db.sendLog.where('leadId').equals(lead.id!).reverse().sortBy('sentAt'),
  ...
  const addNote = async () => {
    ...
    await db.leadNotes.add({
      leadId: lead.id!,
      content: newNote.trim(),
      createdAt: new Date().toISOString(),
    });
    setNewNote('');
    const updated = await db.leadNotes.where('leadId').equals(lead.id!).reverse().sortBy('createdAt');
    setNotes(updated);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-40 flex items-start justify-center pt-4">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col mx-2 animate-scale-in">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {sendLogs.length > 0 && (
            <div>
              <p className="text-xs font-medium text-gray-700 mb-2 border-b pb-1">Historial de envíos</p>
              <div className="space-y-1">
                {sendLogs.map((log) => {
                  const tplName = getTemplateName(log.templateId, log.templateType);
          ...
          {/* Notas - timeline */}
          <div>
            <p className="text-xs font-medium text-gray-700 mb-2 border-b pb-1">Historial de notas</p>
