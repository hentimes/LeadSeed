...
# Add document body overflow toggle
if 'useEffect(() => {' not in content.split('return (')[0]:
    pass # we'll just inject it right before the return block
...
# We'll inject the useEffect for body overflow right before the return
injection_before_return = """
  useEffect(() => {
  ...
  const copyToClipboard = (text: string) => {
    ...
    // Opcional: mostrar un mini toast si tuvieras uno global
  };
"""

new_return = """  return (
    <div onClick={handleBackdropClick} className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-50 flex items-start justify-center pt-8 p-4 overflow-y-auto custom-scrollbar">
      <div className="bg-white rounded-[8px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] w-full max-w-[420px] flex flex-col mx-auto animate-scale-in border border-slate-100 mb-8 relative">
        ...
        <div className="flex-1 p-4 pt-3 space-y-4">
          ...
          <div className="flex flex-col gap-2">
            {lead.phone && (
              <div className="flex items-center gap-2">
                <div 
                  className="flex-1 flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-[6px] cursor-pointer hover:bg-slate-100 transition-colors"
                ...
                <a href={`https://wa.me/${lead.phone.replace(/[^+\d]/g, '')}`} target="_blank" rel="noopener noreferrer" className="w-8 h-8 flex items-center justify-center bg-[#F0FDF4] text-[#16A34A] rounded-[6px] hover:bg-[#DCFCE7] transition-colors border border-[#BBF7D0]" title="WhatsApp">
                  {Icon.Phone()}
                </a>
                <a href={`tel:${lead.phone.replace(/[^+\d]/g, '')}`} className="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-[6px] hover:bg-blue-100 transition-colors border border-blue-200" title="Llamar">
                  {Icon.Phone()}
                </a>
              </div>
            )}
            {lead.email && (
              <div className="flex items-center gap-2">
                ...
                <a href={`mailto:${lead.email}`} className="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-[6px] hover:bg-blue-100 transition-colors border border-blue-200" title="Enviar correo">
                  {Icon.Email()}
                </a>
              </div>
            )}
          ...
          {(isPlanesproLead || journey) && (
            <div>
              ...
              {journey && (
                <div className="bg-[#FAFAFA] rounded-[6px] p-3 border border-slate-100 mb-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {planesproDetails.rangoEdad && (
                  <div className="bg-slate-50 p-2 rounded-[6px] border border-slate-100">
                ...
                {/* Renta condicional: siempre mostrar si existe, y especialmente si es Fonasa */}
              ...
              {(planesproDetails.numeroCargas && planesproDetails.numeroCargas !== '0') && (
                <details className="mt-2 group">
                  <summary className="text-[11px] font-bold text-slate-500 cursor-pointer hover:text-slate-700 list-none flex items-center gap-1 bg-slate-50 p-2 rounded-[6px] border border-slate-100">
                  ...
                  <div className="mt-1 p-2 bg-slate-50 border border-slate-100 rounded-[6px]">
          ...
          <div>
            ...
            {!canCreateAppointment ? (
              <div className="bg-blue-50 border border-blue-100 rounded-[6px] p-2.5 flex justify-between items-center">
                <div>
                ...
                {onNavigate && (
                  <button onClick={() => { openAgendaAppointment(activeAppointment?.id); onClose(); onNavigate('agenda'); }} className="px-3 py-1.5 bg-white border border-blue-200 rounded-[4px] text-[11px] font-bold text-blue-700 shadow-sm hover:bg-blue-100 transition-colors">
                    Ver cita
                  </button>
                )}
              </div>
          ...
          <div>
            ...
            {showNotes && (
              <div className="space-y-2 mt-2">
                {notes.map((note) => (
                  <div key={note.id} className="bg-slate-50 rounded-[6px] p-2.5 border border-slate-100">
...
with open('src/components/leads/LeadDetail.tsx', 'w', encoding='utf-8') as f:
    f.write(content[:start_idx] + injection_before_return + new_return)

print("Premium Redesign Applied Round 3")
